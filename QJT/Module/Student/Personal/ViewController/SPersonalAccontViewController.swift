...
extension SPersonalAccontViewController {
    func logoutBtnClicked() {
        ...
        let confirmAction = UIAlertAction(title: "确定", style: UIAlertActionStyle.Default) { (action) in
            NetWorkManager.httpRequest(Methods.login_studentLogout, params: ["studentID":UserConfig.studentSetting()!.userID], modelType: EmptyModel(), listType: EmptyModel(), completed: { (responseData) in
