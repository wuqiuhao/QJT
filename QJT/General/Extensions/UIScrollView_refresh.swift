...
extension UIScrollView {
    ...
    var configRefreshDelegate: ConfigRefreshDelegate {
        ...
        set {
            objc_setAssociatedObject(self, &tConfigRefreshDelegate, newValue as! UIViewController, objc_AssociationPolicy.OBJC_ASSOCIATION_ASSIGN)
        }
    ...
    var headerRefresh: Bool {
        set {
            ...
            if newValue == true {
                ...
                self.mj_header = AnimationRefresh(refreshingBlock: { () -> Void in
                    self.demandPage = 1
                    self.configRefreshDelegate.headerRefresh!(self)
                })
                self.mj_header.beginRefreshing()
                self.mj_header.automaticallyChangeAlpha = true
